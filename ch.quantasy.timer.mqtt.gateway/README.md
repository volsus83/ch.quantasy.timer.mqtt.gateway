...
There is only one intent needed, in order to setup or alter a timer: 
```sh
Topic: Time/Tick/[instance]/I/configuration
Message: --- 
         id: <String>
 ...
 * first: first occurence in UNIX epoch (milliseconds since 01.01.1970). Can be set to any time in the past (e.g. 0) in order to start immediately.
If first is not given, ticking starts immediately.
 * interval: interval between two 'ticks' in milliseconds. If interval is not given, no intermediate ticks will be given
 * last: relative time for the last tick. The time is relative to the first tick, e.g. if last would be set to 10000 the last tick would be sent 10 seconds after the first.
